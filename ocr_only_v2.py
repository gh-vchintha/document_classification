#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
Combined OCR + Classification pipeline (single file) — with Bedrock timeout fixes

- Per-page OCR with Amazon Nova (Bedrock), memory-safe rendering
- Per-PDF classification (NO truncation) inside batch_process_pdfs
- Returns rows: pdf_file, results, classification, job_run_time  (all strings)
- Parquet writer with explicit all-string schema
- Robust logging to stdout with periodic progress heartbeats
- Timeout hardening:
  • Bedrock client uses longer read/connect timeouts
  • OCR retries downscale/recompress image adaptively on timeout
  • Classification also uses retries

Run:
  python main.py             # batch mode (writes Parquet to S3)
  python main.py local       # local mode (writes CSV locally)

Env:
  OUTPUT_BUCKET / output_bucket (required for batch)
  AWS_REGION (default us-east-1)
  NOVA_MODEL_ID (default us.amazon.nova-pro-v1:0)

  # Rendering & concurrency
  PDF_RENDER_DPI=220         (render speed/quality knob)
  PDF_RENDER_FMT=jpeg        (jpeg|png)
  PDF_OCR_MAX_WORKERS=6      (per-PDF thread pool)
  PDF_OCR_MAX_OCR=4          (concurrent Bedrock calls per PDF)
  PDF_OCR_MAX_PDFS=3         (PDFs in parallel at job level)
  PDF_OCR_MAX_RETRIES=4      (OCR retries)
  CLS_MAX_RETRIES=3          (classification retries)

  # Progress logging
  OCR_PROGRESS_PAGES=5       (log heartbeat every N completed pages)
  OCR_PROGRESS_SECS=20       (or every M seconds)
  LOG_LEVEL=INFO
  RUN_MODE=local             (optional; "local" to run local_run())

  # Bedrock timeouts
  BEDROCK_READ_TIMEOUT=180
  BEDROCK_CONNECT_TIMEOUT=10

  # OCR image sizing (first try vs fallback on retry)
  OCR_MAX_SIDE=2000
  OCR_MAX_SIDE_FALLBACK=1400
  OCR_JPEG_QUALITY=85
  OCR_JPEG_QUALITY_FALLBACK=75

  # Container tip
  PYTHONUNBUFFERED=1         (recommended in container for live logs)
"""

import base64
import gc
import io
import json
import os
import random
import shutil
import tempfile
import threading
import time
from typing import Any, Dict, List, Optional, Tuple
from urllib.parse import urlparse

import boto3
import botocore.exceptions
import pandas as pd
import pyarrow as pa
import pyarrow.parquet as pq
from PIL import Image
from openai import OpenAI  # used only in transcribe_handwritten()
from pdf2image import convert_from_bytes, pdfinfo_from_bytes

# Guardant PoC bits (assumed available)
from poc.client import api_key
from poc.promts import EXTRACTION_PROMPT
from poc.promts import get_benchmark_prompt, classify_v3_prompt

openai_client = OpenAI(api_key=api_key)
import cv2
import numpy as np

# ------------------------------------------------------------------------------
# Robust logging setup (forces stdout handler)
# ------------------------------------------------------------------------------
# put this near the top of ocr_only_v2.py, before any logging calls
import logging

LOG_LEVEL = "INFO"  # or wherever you set it

logging.basicConfig(
    level=getattr(logging, LOG_LEVEL, logging.INFO),
    format="%(asctime)s | %(levelname)s | %(threadName)s | %(name)s | %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
    force=True,  # override any prior handlers/config
)

# Optional: reduce third-party chatter (httpx/httpcore)
logging.getLogger("httpx").setLevel(logging.WARNING)
logging.getLogger("httpcore").setLevel(logging.WARNING)

logger = logging.getLogger(__name__)


CLASSIFICATION_OPENAI_MODEL = os.environ.get("CLASSIFICATION_OPENAI_MODEL", "gpt-5")
# ------------------------------------------------------------------------------
# Render tunables
# ------------------------------------------------------------------------------
PDF_RENDER_DPI = int(os.environ.get("PDF_RENDER_DPI", "220"))
PDF_RENDER_FMT = os.environ.get("PDF_RENDER_FMT", "jpeg")  # jpeg|png

# ------------------------------------------------------------------------------
# Bedrock timeout & OCR image size knobs
# ------------------------------------------------------------------------------
BEDROCK_READ_TIMEOUT = int(os.environ.get("BEDROCK_READ_TIMEOUT", "300"))
BEDROCK_CONNECT_TIMEOUT = int(os.environ.get("BEDROCK_CONNECT_TIMEOUT", "10"))

OCR_MAX_SIDE = int(os.environ.get("OCR_MAX_SIDE", "2000"))
OCR_MAX_SIDE_FALLBACK = int(os.environ.get("OCR_MAX_SIDE_FALLBACK", "1400"))
OCR_JPEG_QUALITY = int(os.environ.get("OCR_JPEG_QUALITY", "85"))
OCR_JPEG_QUALITY_FALLBACK = int(os.environ.get("OCR_JPEG_QUALITY_FALLBACK", "75"))

CLS_MAX_RETRIES = int(os.environ.get("CLS_MAX_RETRIES", "3"))

# ------------------------------------------------------------------------------
# Parquet schema (job_run_time now real timestamp)
# ------------------------------------------------------------------------------
DOC_SCHEMA = pa.schema([
    pa.field("pdf_file", pa.string()),
    pa.field("results", pa.string()),
    pa.field("classification", pa.string()),
    pa.field("bench_mark", pa.string()),
    pa.field("job_run_time", pa.timestamp("s")),
])

def write_results_parquet_s3(rows: List[Dict[str, Any]], bucket: str, key: str):
    """
    Writes:
      - Local JSON lines (output.json) for debugging
      - Parquet to s3://{bucket}/{key} with DOC_SCHEMA
    """
    df = pd.DataFrame(rows, columns=["pdf_file", "results", "classification", "bench_mark", "job_run_time"])
    # Ensure correct dtypes for declared schema
    df = df.astype({
        "pdf_file": "string",
        "results": "string",
        "classification": "string",
        "bench_mark": "string",
    })
    # Convert job_run_time to pandas datetime if present (allow nulls)
    if "job_run_time" in df.columns:
        df["job_run_time"] = pd.to_datetime(df["job_run_time"], errors="coerce", utc=True)

    # Drop a local JSONL for inspection
    with open("output.json", "w", encoding="utf-8") as f:
        for _, row in df.iterrows():
            f.write(json.dumps({k: (row[k].isoformat() if k == "job_run_time" and pd.notna(row[k]) else row[k])
                                for k in df.columns}, ensure_ascii=False) + "\n")
    logger.info("Saved JSON lines to output.json (rows=%d)", len(rows))

    # Write Parquet to an in-memory buffer with explicit schema then upload to S3
    table = pa.Table.from_pandas(df, schema=DOC_SCHEMA, preserve_index=False)
    buf = io.BytesIO()
    pq.write_table(table, buf)
    buf.seek(0)

    s3 = boto3.client("s3")
    s3.put_object(Bucket=bucket, Key=key, Body=buf.getvalue())
    logger.info("Saved Parquet to s3://%s/%s (rows=%d)", bucket, key, len(rows))

# ------------------------------------------------------------------------------
# S3 helpers
# ------------------------------------------------------------------------------
def parse_s3_uri(uri: str) -> Tuple[str, str]:
    u = urlparse(uri)
    if u.scheme != "s3" or not u.netloc or not u.path:
        raise ValueError(f"Invalid S3 URI: {uri!r}. Expected: s3://<bucket>/<key>")
    return u.netloc, u.path.lstrip("/")

def get_bytes_from_s3(s3_uri: str) -> bytes:
    bucket, key = parse_s3_uri(s3_uri)
    s3 = boto3.client("s3")
    resp = s3.get_object(Bucket=bucket, Key=key)
    return resp["Body"].read()

# ------------------------------------------------------------------------------
# Extractor + Classifier
# ------------------------------------------------------------------------------
class HandwritingExtractor:
    def __init__(self, aws_region: Optional[str] = None, model_id: Optional[str] = None):
        # Keep defaults consistent with docstring
        self.aws_region = aws_region or os.environ.get("AWS_REGION") or "us-east-1"
        self.model_id = model_id or os.environ.get("NOVA_MODEL_ID", "us.amazon.nova-pro-v1:0")
        self.classification_model_id = os.environ.get("CLASSIFICATION_MODEL_ID", "us.amazon.nova-premier-v1:0")

        self._local = threading.local()  # thread-local Bedrock client

        # Concurrency knobs
        self.max_workers = int(os.environ.get("PDF_OCR_MAX_WORKERS", "6"))
        self.max_ocr_workers = int(os.environ.get("PDF_OCR_MAX_OCR", "12"))
        self.max_retries = int(os.environ.get("PDF_OCR_MAX_RETRIES", "4"))
        self._ocr_sem = threading.Semaphore(self.max_ocr_workers)


    # ---------------- OCR image prep + model call ----------------
    @staticmethod
    def _prep_for_ocr(image: Image.Image, max_side: int = OCR_MAX_SIDE, jpeg_quality: int = OCR_JPEG_QUALITY) -> Tuple[str, str]:
        im = image.convert("L")
        w, h = im.size
        m = max(w, h)
        if m > max_side:
            scale = max_side / float(m)
            im = im.resize((int(w * scale), int(h * scale)), Image.LANCZOS)
        buf = io.BytesIO()
        im.save(buf, format="JPEG", quality=jpeg_quality, optimize=True)
        b64 = base64.b64encode(buf.getvalue()).decode("utf-8")
        return b64, "jpeg"

    def bench_mark_wrapper(self , filename , model_results):
        try:
            start_time = time.time()
            actual_results = pd.read_csv("/Users/vchintha/Documents/GitHub/openai/poc/actuals/"+filename.split("/")[-1].replace(".pdf",".csv")).to_json()
            bench_mark = self.bench_mark_classification(actual_results, model_results)
            elapsed = time.time() - start_time
            logger.info("Bench mark completed in %.2fs", elapsed)
            return bench_mark
        except Exception as e:
            logger.error("Bench mark failed for %s: %s", filename, e)
            return ""

    def bench_mark_classification(self, actual_results, model_results, temperature: float = 0.1) -> str:
        prompt = get_benchmark_prompt(actual_results, model_results).strip()
        delay = 0.5
        for attempt in range(1, CLS_MAX_RETRIES + 1):
            try:
                resp = openai_client.responses.create(model=CLASSIFICATION_OPENAI_MODEL,
                                                      input=[
                                                          {"role": "system", "content": "Return only a single JSON array. No prose, no Markdown, no comments."},
                                                          {"role": "user", "content": prompt},
                                                      ],
                                                      )
                # results = resp.choices
                all_text = resp.output_text
                return all_text
            except Exception as e:
                if attempt < CLS_MAX_RETRIES:
                    sleep_for = delay * (2 ** (attempt - 1)) + random.uniform(0, 0.25)
                    logger.warning("Benchmark attempt %d failed (%s); retrying in %.2fs…", attempt, e, sleep_for)
                    time.sleep(sleep_for)
                else:
                    raise

    def preprocess_image(self, img_bytes):
        start_time = time.time()
        nparr = np.frombuffer(img_bytes, np.uint8)
        img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
        # Upscale image
        img = cv2.resize(img, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC)
        # Convert to grayscale
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        # Invert colors
        inverted = cv2.bitwise_not(gray)
        # Adaptive thresholding
        thresh = cv2.adaptiveThreshold(
            inverted, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
            cv2.THRESH_BINARY, 11, 2
        )
        # Save the thresholded image for debugging
        # debug_dir = "debug_images"
        # os.makedirs(debug_dir, exist_ok=True)
        # uuid = np.random.randint(1000000)
        # debug_path = os.path.join(debug_dir, f"preprocessed{uuid}.png")
        # cv2.imwrite(debug_path, thresh)
        _, buffer = cv2.imencode('.png', thresh)
        elapsed = time.time() - start_time
        logger.info("Image preprocessed in %.2fs", elapsed)
        return buffer.tobytes()

    def prepare_image_for_vision(self, img_bytes: bytes, max_side: int = 1400, jpeg_quality: int = 85) -> bytes:
        img = Image.open(io.BytesIO(img_bytes)).convert("L")  # grayscale helps OCR & size
        w, h = img.size
        scale = max(w, h) / max_side if max(w, h) > max_side else 1.0
        if scale > 1.0:
            img = img.resize((int(w/scale), int(h/scale)), Image.LANCZOS)
        out = io.BytesIO()
        img.save(out, format="JPEG", quality=jpeg_quality, optimize=True)
        return out.getvalue()

    def transcribe_handwritten(self, img_bytes, prompt):
        # processed_img = self.preprocess_image(img_bytes)
        processed_img = self.prepare_image_for_vision(img_bytes)
        # jpeg_bytes, meta = auto_orient_and_deskew(img_bytes)
        b64 = base64.b64encode(processed_img).decode("utf-8")

        system_msg = {
            "role": "system",
            "content": (
                "You are a vision-enabled assistant. "
                "Look carefully at the provided image(s), analyze handwriting, if page is not human readable, don't attempt to guess, get empty string. "
                "and always return a valid JSON object only."
            )
        }

        resp = openai_client.chat.completions.create(
            model="gpt-4o",
            response_format={ "type": "json_object" },
            messages=[system_msg,{
                "role": "user",
                "content": [
                    {"type": "text", "text": f"{prompt}:: . provide output in JSON format only No other text"},
                    {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{b64}", "detail": "high"}}
                ]
            }]
        )
        all_res = resp.choices
        all_text = "\n\n".join(self._strip_code_fences(choice.message.content or "") for choice in all_res)
        return all_text

    def extract_text_from_image(self, image: Image.Image, custom_prompt: Optional[str] = None) -> str:
        """
        Invoke Nova with adaptive retry:
        - 1st attempt uses OCR_MAX_SIDE / OCR_JPEG_QUALITY
        - On timeout/conn errors, retry with smaller max_side & lower quality
        """
        with self._ocr_sem:
            delay = 0.5
            max_side = OCR_MAX_SIDE
            quality = OCR_JPEG_QUALITY

            for attempt in range(1, self.max_retries + 1):
                if attempt > 1:
                    max_side = min(max_side, OCR_MAX_SIDE_FALLBACK)
                    quality = min(quality, OCR_JPEG_QUALITY_FALLBACK)

                b64, fmt = self._prep_for_ocr(image, max_side=max_side, jpeg_quality=quality)
                request_body = {
                    "schemaVersion": "messages-v1",
                    "messages": [{
                        "role": "user",
                        "content": [
                            {"image": {"format": fmt, "source": {"bytes": b64}}},
                            {"text": custom_prompt or EXTRACTION_PROMPT}
                        ],
                    }],
                    "inferenceConfig": {"temperature": 0.1, "topP": 0.0},
                }

                t0 = time.perf_counter()
                try:
                    br = self._get_bedrock_client()
                    resp = br.invoke_model(
                        modelId=self.model_id,
                        body=json.dumps(request_body),
                        accept="application/json",
                        contentType="application/json",
                    )
                    data = json.loads(resp["body"].read())
                    out_msg = data.get("output", {}).get("message", {})
                    for item in out_msg.get("content", []) or []:
                        if isinstance(item, dict) and "text" in item:
                            text = (item["text"] or "").strip()
                            return text
                    raise ValueError(f"OCR: Nova returned no text. Raw: {json.dumps(data)[:1000]}")

                except (botocore.exceptions.ReadTimeoutError,
                        botocore.exceptions.ConnectTimeoutError,
                        botocore.exceptions.EndpointConnectionError) as e:
                    if attempt < self.max_retries:
                        sleep_for = delay * (2 ** (attempt - 1)) + random.uniform(0, 0.25)
                        logger.warning(
                            "OCR timeout/connection on attempt %d (max_side=%s q=%s): %s; retrying in %.2fs…",
                            attempt, max_side, quality, e, sleep_for
                        )
                        time.sleep(sleep_for); continue
                    logger.error("OCR failed after %d attempts due to timeouts.", attempt); raise
                except Exception as e:
                    if attempt < self.max_retries:
                        sleep_for = delay * (2 ** (attempt - 1)) + random.uniform(0, 0.25)
                        logger.warning("OCR attempt %d failed (%s); retrying in %.2fs…", attempt, e, sleep_for)
                        time.sleep(sleep_for)
                    else:
                        raise

    # ---------------- Render one page ----------------
    def _render_single_page(self, pdf_bytes: bytes, page: int,
                            dpi: int = PDF_RENDER_DPI, fmt: str = PDF_RENDER_FMT,
                            grayscale: bool = True) -> Image.Image:
        tmpdir = tempfile.mkdtemp(dir="/tmp")
        try:
            paths = convert_from_bytes(
                pdf_bytes, dpi=dpi, fmt=fmt, grayscale=grayscale,
                output_folder=tmpdir, paths_only=True, thread_count=1,
                first_page=page, last_page=page,
            )
            if not paths:
                raise RuntimeError(f"Failed to render page {page}")
            p = paths[0]
            try:
                im = Image.open(p)
                return im.copy()
            finally:
                try: os.remove(p)
                except OSError: pass
        finally:
            shutil.rmtree(tmpdir, ignore_errors=True)

    # ---------------- Process one page task ----------------
    def _process_page_task(self, pdf_bytes: bytes, page_num: int, custom_prompt: Optional[str]) -> Tuple[int, str]:
        t0 = time.perf_counter()
        img = None
        try:
            img = self._render_single_page(pdf_bytes, page_num)
            # Convert PIL Image -> PNG bytes for transcribe_handwritten()
            buf = io.BytesIO()
            img.save(buf, format="PNG")
            img_bytes = buf.getvalue()
            prompt = custom_prompt or EXTRACTION_PROMPT
            text = self.transcribe_handwritten(img_bytes, prompt)
            text = self._strip_code_fences(text or "")
            return page_num, text
        finally:
            try:
                if img is not None:
                    img.close()
            except Exception:
                pass
            gc.collect()
            elapsed = time.perf_counter() - t0
            logger.info("Page %d done in %.2fs", page_num, elapsed)

    # ---------------- Process a PDF → dict(page->text) ----------------
    def process_pdf(self, pdf_bytes: bytes, custom_prompt: Optional[str] = None , max_pages :Optional[str] = None ) -> Dict[int, str]:
        from concurrent.futures import ThreadPoolExecutor, as_completed

        info = pdfinfo_from_bytes(pdf_bytes)
        num_pages = int(max_pages) if max_pages is not None else int(info.get("Pages", 0)) or 0

        if num_pages <= 0:
            raise ValueError("Could not determine page count (pdfinfo).")
        workers = max(2, min(self.max_workers, num_pages))
        logger.info("OCR: pages=%d | workers=%d | ocr_concurrency=%d", num_pages, workers, self.max_ocr_workers)

        results: Dict[int, str] = {}

        # Heartbeat configuration
        heartbeat_every_pages = int(os.environ.get("OCR_PROGRESS_PAGES", "5"))
        heartbeat_every_secs = float(os.environ.get("OCR_PROGRESS_SECS", "20"))
        done = 0
        t_start = time.perf_counter()
        last_log = t_start

        with ThreadPoolExecutor(max_workers=workers) as ex:
            futs = {
                ex.submit(self._process_page_task, pdf_bytes, p, custom_prompt): p
                for p in range(1, num_pages + 1)
                # for p in range(68,90)
            }
            for fut in as_completed(futs):
                page, text = fut.result()
                results[page] = text
                done += 1

                now = time.perf_counter()
                if (done % heartbeat_every_pages == 0) or (now - last_log >= heartbeat_every_secs):
                    rate = done / max(1e-6, (now - t_start))
                    remaining = num_pages - done
                    eta_s = remaining / max(1e-6, rate)
                    logger.info("Progress: %d/%d pages (%.1f%%), ~%.2f p/s, ETA ~%.0fs",
                                done, num_pages, 100.0 * done / num_pages, rate, eta_s)
                    last_log = now

        total_s = time.perf_counter() - t_start
        logger.info("OCR complete: %d/%d pages in %.2fs (%.2f p/s)",
                    done, num_pages, total_s, done / max(1e-6, total_s))
        return {k: results[k] for k in sorted(results)}

    # ---------------- Classification helpers (NO truncation) ----------------
    @staticmethod
    def _strip_code_fences(s: str) -> str:
        s = s.strip()
        if s.startswith("```"):
            s = s.split("\n", 1)[-1]
        if s.endswith("```"):
            s = s.rsplit("\n", 1)[0]
        s = s.strip()
        if s.lower().startswith("json"):
            s = s.split("\n", 1)[-1].strip()
        return s

    def _build_pages_array_no_trunc(
            self,
            extracted_per_page: Dict[int, str],
            kv_per_page: Optional[Dict[int, Any]] = None,
    ) -> List[Dict[str, Any]]:
        pages: List[Dict[str, Any]] = []
        kv_per_page = kv_per_page or {}
        for p in sorted(extracted_per_page):
            pages.append({
                "pageno": int(p),
                "rawtext": extracted_per_page[p] or "",
                "data": kv_per_page.get(p, {}) or {},
            })
        return pages

    def _classify_pages_no_trunc(self, pages: List[Dict[str, Any]], *, temperature: float = 0.1) -> str:
        pages_text = json.dumps(pages, ensure_ascii=False, separators=(",", ":"))
        prompt = classify_v3_prompt(pages_text).strip()

        delay = 0.5
        for attempt in range(1, CLS_MAX_RETRIES + 1):
            try:
                resp = openai_client.responses.create(model=CLASSIFICATION_OPENAI_MODEL,
                                                      input=[
                                                          {"role": "system", "content": "Return only a single JSON array. No prose, no Markdown, no comments."},
                                                          {"role": "user", "content": prompt},
                                                      ],
                                                      )
                # results = resp.choices
                all_text = resp.output_text
                return all_text

            except (botocore.exceptions.ReadTimeoutError,
                    botocore.exceptions.ConnectTimeoutError,
                    botocore.exceptions.EndpointConnectionError) as e:
                if attempt < CLS_MAX_RETRIES:
                    sleep_for = delay * (2 ** (attempt - 1)) + random.uniform(0, 0.25)
                    logger.warning("Classification timeout on attempt %d: %s; retrying in %.2fs…", attempt, e, sleep_for)
                    time.sleep(sleep_for); continue
                logger.error("Classification failed after %d attempts due to timeouts.", attempt); raise
            except Exception as e:
                if attempt < CLS_MAX_RETRIES:
                    sleep_for = delay * (2 ** (attempt - 1)) + random.uniform(0, 0.25)
                    logger.warning("Classification attempt %d failed (%s); retrying in %.2fs…", attempt, e, sleep_for)
                    time.sleep(sleep_for)
                else:
                    raise

    # ---------------- Batch: OCR + classify (in one method) ----------------
    def batch_process_pdfs(self, pdf_list: List[str], custom_prompt: str, is_classify: bool = True,max_pages_to_process:int = None) -> List[Dict[str, Any]]:
        """
        For each PDF:
          1) OCR all pages (dict page->text)
          2) Build pages array (NO truncation) and classify with Nova
          3) Return row of strings: pdf_file, results, classification, job_run_time
        """
        from concurrent.futures import ThreadPoolExecutor

        def _load_pdf(path: str) -> bytes:
            return get_bytes_from_s3(path) if path.lower().startswith("s3://") else open(path, "rb").read()

        def _one_pdf(pdf_path: str) -> Dict[str, Any]:
            filename_ = pdf_path.split("/")[-1]
            try:
                pdf_bytes = _load_pdf(pdf_path)
                logger.info("Processing %s", pdf_path)

                # 1) OCR for THIS PDF
                extracted = self.process_pdf(pdf_bytes, custom_prompt , max_pages=max_pages_to_process)

                # 2) Build pages (NO truncation) + classify
                pages = self._build_pages_array_no_trunc(extracted, kv_per_page=None)
                logger.info("Classify start: %s (pages=%d)", pdf_path, len(extracted))
                with open(f"/Users/vchintha/Documents/GitHub/openai/poc/outputs/debug/{filename_}.txt", "w") as file:
                    file.write(str(pages))
                # write pages into file_name txt document

                if is_classify:
                    classification_json =self._classify_pages_no_trunc(pages, temperature=0.1)
                else:
                    classification_json = {}
                logger.info("Classify done: %s", pdf_path)

                # job_run_time as real UTC timestamp; results as compact JSON string
                job_run_time = pd.Timestamp.utcnow().tz_convert("UTC")

                return {
                    "filename": str(filename_),
                    "classification": classification_json,
                    "bench_mark": "actuals",
                    "job_run_time": job_run_time,
                }

            except Exception as e:
                logger.error("Error processing %s: %s", pdf_path, e)
                # keep types consistent; allow null job_run_time if parsing fails
                return {
                    "filename": str(filename_),
                    "classification": f"Error: {e}",
                    "bench_mark": f"Error: {e}",
                    "job_run_time": pd.NaT,
                }

        max_pdf_workers = int(os.environ.get("PDF_OCR_MAX_PDFS", "3"))
        logger.info("Starting batch of %d PDFs (parallel=%d)", len(pdf_list), max_pdf_workers)
        rows: List[Dict[str, Any]] = []
        with ThreadPoolExecutor(max_workers=max_pdf_workers) as ex:
            for r in ex.map(_one_pdf, pdf_list):
                rows.append(r)
                logger.info("Completed PDF: %s", r.get("pdf_file"))
        logger.info("Batch finished: %d PDFs", len(rows))
        return rows

# ----------------------------- Runners ----------------------------------------


def local_run():
    extractor = HandwritingExtractor(aws_region=os.environ.get("AWS_REGION", "us-east-1"))
    try:
        input_folder = "/Users/vchintha/Documents/GitHub/openai/poc/inputs/"
        files = os.listdir(input_folder)
        paths = []
        paths_ = []
        for file_ in files:
            paths.append(f'{input_folder}{file_}')
            paths_.append(file_)

        # print(paths_)
        #
        # return
        custom_prompt = EXTRACTION_PROMPT
        results_rows = extractor.batch_process_pdfs(paths, custom_prompt,is_classify=True,max_pages_to_process = 100)
        current_time = pd.Timestamp.utcnow().strftime("%Y%m%d_%H%M%S")
        pd.DataFrame(results_rows).to_csv(f"outputs/all_transcriptions.csv", index=False)
        logger.info("Wrote output.csv (%d rows)", len(results_rows))
    except Exception as e:
        logger.error("Local error: %s", e)
        raise

if __name__ == "__main__":
    local_run()